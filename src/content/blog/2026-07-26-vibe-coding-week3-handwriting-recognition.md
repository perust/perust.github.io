...
description: "혼자 공부하는 바이브 코딩 with 클로드 코드 3주차 기록입니다. 두 번의 프롬프트로 만든 손글씨 인식 프로그램과 웹 버전을 연속 숫자 입력 도구로 개선한 과정을 정리했습니다."
date: "2026-07-26"
category: "도서 학습 챌린지"
...
## 웹 버전 개선: 한 자리에서 연속 숫자 입력까지

웹 버전을 만든 뒤 직접 써보니, 숫자를 한 번 인식하고 끝나는 화면보다 실제 입력 도구처럼 사용할 수 있으면 좋겠다는 생각이 들었습니다. 처음부터 연속 입력 기능을 만든 것은 아니고, 사용하면서 불편했던 점을 하나씩 고쳤습니다.

### 1차: 한 자리 숫자 인식

1차 웹 버전은 정사각형 캔버스에 숫자 하나를 쓰면 예측값과 숫자별 점수를 보여주는 구조였습니다. 아래 화면에서는 손글씨 7을 인식했고, 화면에 표시된 신뢰도는 74.0%였습니다. 모델에 들어간 28×28 이미지도 함께 확인할 수 있었습니다.

![1차 웹 버전이 손글씨 숫자 7을 인식하고 숫자별 점수를 표시한 화면](/images/posts/2026-07-26-vibe-coding-week3/09-web-single-digit.webp)

숫자를 인식하는 기능은 잘 작동했지만 결과를 기록하려면 사용자가 다시 입력해야 했습니다. 인식 결과를 바로 텍스트로 모을 수 있는 입력창이 필요했습니다.

### 2차: 확인한 숫자를 입력창에 추가

다음 버전에서는 `Digits` 입력창을 만들었습니다. 캔버스에 쓴 숫자가 맞게 인식됐는지 확인한 뒤 Enter 키나 `Add` 버튼을 누르면 입력창 뒤에 숫자가 붙도록 했습니다. 잘못 입력한 값을 지우거나 결과를 복사할 수 있도록 `Copy`, `Erase` 버튼도 추가했습니다.

![손글씨 숫자 4를 확인한 뒤 여러 숫자를 Digits 입력창에 모은 화면](/images/posts/2026-07-26-vibe-coding-week3/10-web-manual-collect.webp)

이제 인식한 숫자를 다시 타이핑할 필요는 없었지만, 숫자를 쓸 때마다 Enter나 버튼을 누르는 과정은 여전히 번거로웠습니다.

### 3차: 0.6초 멈추면 자동 입력

펜을 뗀 뒤 0.6초 동안 추가 입력이 없으면 인식한 숫자를 자동으로 입력창에 넣도록 바꿨습니다. 필요할 때만 사용할 수 있도록 `Add on its own after a 0.6 second pause` 옵션도 만들었습니다.

![0.6초 동안 입력이 없으면 인식한 숫자를 자동으로 추가하는 옵션](/images/posts/2026-07-26-vibe-coding-week3/11-web-auto-commit.webp)

한 자리씩 쓰고 기다리는 방식까지는 편해졌지만, 여러 숫자를 가로로 이어 쓰고 싶다는 생각이 들었습니다. 작은 정사각형 캔버스보다는 글씨를 옆으로 이어갈 수 있는 긴 캔버스가 더 자연스러워 보였습니다.

### 4차: 긴 캔버스에서 연속 숫자 인식

처음에는 숫자 사이의 시간 간격을 정하고, 약 2초 전에 그린 흔적부터 인식한 뒤 서서히 지우는 방식을 생각했습니다. 한 번에 긴 숫자 전체를 판별하는 대신 끝난 숫자부터 하나씩 확정하면, 매번 인식해야 하는 범위를 줄일 수 있을 것 같았습니다.

실제로 구현하면서 숫자의 경계를 시간만으로 나누지는 않았습니다. 사람마다 쓰는 속도가 다르고, 4처럼 여러 획으로 쓰는 숫자가 중간에 끊길 수 있기 때문입니다. 현재 버전은 획 사이의 가로 간격으로 숫자를 나눕니다. 오른쪽에 다음 숫자를 쓰기 시작하면 앞의 숫자를 먼저 입력창에 넣고, 인식이 끝난 흔적은 약 0.45초 동안 서서히 사라집니다.

![긴 캔버스에 숫자를 연속으로 쓰고 인식 결과를 입력창에 모은 화면](/images/posts/2026-07-26-vibe-coding-week3/12-web-continuous-strip.webp)

마지막 숫자는 오른쪽에 다음 숫자가 없으므로 잠시 기다린 뒤 자동으로 확정됩니다. 그림을 캡처했을 때는 0.6초로 설정했지만, 현재 공개한 버전에서는 대기 시간을 직접 바꿀 수 있고 기본값은 0.8초입니다. `Take now`, `Undo`, `Copy`, `Erase` 버튼으로 즉시 입력하거나 마지막 숫자를 취소하고 결과를 복사하거나 지울 수도 있습니다.

처음에는 숫자 하나를 맞히는 데 집중했지만, 몇 번 써보니 인식 결과를 언제 확정하고 다음 숫자와 어떻게 나눌지가 더 불편하게 느껴졌습니다. 모델을 바꾸지 않아도 입력창, 자동 확정, 긴 캔버스를 차례로 더하자 단순한 시연 화면에서 연속 숫자를 입력할 수 있는 도구에 가까워졌습니다.
