...
이번 주에는 『혼자 공부하는 바이브 코딩 with 클로드 코드』 Ch 03을 공부하고, 손글씨 숫자를 인식하는 프로그램을 만들었습니다. 3주차 기본 미션은 PROJECT 3의 프로그램을 완성하고 실행 화면을 캡처하는 것입니다.

이 실습은 예전 버전의 AI로도 진행한 적이 있습니다. 그래서 같은 미션을 지금 버전으로 다시 해보면 어느 정도 달라졌는지 궁금했습니다. 결과부터 말하면 이번에는 프로그램 제작과 MNIST 학습, macOS 실행 앱 생성까지 단 두 번의 프롬프트로 끝났습니다.
...
이후 실행 파일도 만들어 달라고 한 번 더 요청했습니다. 손글씨 인식 프로그램을 만들어 달라는 프롬프트와 실행 파일을 만들어 달라는 프롬프트, 이렇게 두 번이 전부였습니다.

처음 요청을 받은 Claude Code는 바로 코드를 쓰기 전에 환경부터 확인했습니다. TensorFlow, PIL, tkinter를 사용할 수 있는지 점검했고, MNIST를 내려받을 수 있는지도 확인했습니다. macOS Python에서 발생한 SSL 인증서 문제도 발견한 뒤 `certifi`를 이용해 해결을 시도했습니다.

예전에는 사용자가 세부 단계를 계속 알려줘야 한다고 느꼈는데, 지금은 필요한 준비와 문제를 먼저 찾고 다음 작업을 이어갔습니다. MNIST 다운로드 여부까지 자동으로 확인하는 모습을 보니 모델 성능이 올라간 것이 실감났습니다.

## 첫 프롬프트에서 MNIST 학습까지

Claude Code는 프로그램의 화면만 만든 것이 아니라 손글씨를 실제로 인식할 수 있도록 MNIST 학습까지 진행했습니다. 화면에는 코드 작성과 전처리 검증을 마친 뒤 12에포크 학습을 백그라운드에서 진행하는 과정이 표시됐습니다.

![코드 구조를 만든 뒤 MNIST 학습을 백그라운드에서 진행하는 화면](/images/posts/2026-07-26-vibe-coding-week3/06-mnist-training.webp)

생성된 구조도 기능별로 나뉘어 있었습니다.
...
프롬프트는 짧았지만 내부에서는 입력 이미지 전처리, 모델 정의, 학습, 예측, 화면 구성이 계층적으로 분리됐습니다. 제가 파일별 역할을 하나씩 지시하지 않아도 Claude Code가 프로젝트 구조를 먼저 잡고 작업을 이어갔습니다.

## 실행 앱까지 완성

두 번째 프롬프트에서는 프로그램을 macOS에서 바로 열 수 있는 실행 앱으로 만들어 달라고 요청했습니다. 완성된 `Study-01` 폴더에는 소스 코드와 학습 모델뿐 아니라 테스트, 도구, 샘플, README, 요구사항 파일, 실행 로그가 정리됐습니다. `Digit Recognizer.app`도 함께 생성됐습니다.

![소스 코드와 모델, 테스트, 실행 앱이 생성된 프로젝트 폴더](/images/posts/2026-07-26-vibe-coding-week3/07-app-bundle.webp)

실행 결과도 정확했습니다. 캔버스에 숫자 5를 그리자 프로그램은 5로 인식했고, 화면에 표시된 신뢰도는 98.8%였습니다. 오른쪽에는 0부터 9까지 각 숫자의 점수가 막대로 표시됐고, 아래에는 실제 모델에 입력된 28×28 이미지도 함께 나왔습니다.

![현재 버전 프로그램이 손글씨 숫자 5를 98.8% 신뢰도로 인식한 결과](/images/posts/2026-07-26-vibe-coding-week3/08-current-result.webp)

제가 그린 숫자 5는 글꼴처럼 반듯하지 않았지만 결과는 정확했습니다. 입력 그림이 모델용 28×28 이미지로 어떻게 바뀌었는지까지 한 화면에서 볼 수 있어서 단순히 정답 숫자만 표시되던 이전 결과보다 프로그램의 동작을 이해하기 쉬웠습니다.
...
지금은 첫 프롬프트에서 환경을 확인하고, MNIST를 준비하고, 코드를 기능별로 나누고, 학습까지 이어졌습니다. 여기에 `'auto mode'`가 생겨 지시한 뒤 계속 지켜보지 않아도 필요한 작업을 이어서 마무리한다는 점도 차이를 크게 느끼게 했습니다.

## 책이 나온 뒤 달라진 AI

이 책은 2025년 12월에 발행됐습니다. 몇 달 사이에 책에서 설명하는 화면과 구조가 많이 바뀌었고, AI 모델의 버전과 성능도 크게 올라갔습니다. 같은 미션을 다시 해보니 AI의 발전 속도가 새삼 빠르다고 느껴졌습니다.

예전에는 프로그램이 겉으로 실행되는지 확인한 뒤, 인식이 이상하면 원인을 다시 설명하고 학습을 추가해야 했습니다. 이번에는 두 번의 프롬프트만으로 학습 모델과 실행 앱까지 만들어졌습니다. 단순히 코드가 조금 더 잘 나온 정도가 아니라, AI가 작업에 필요한 단계를 스스로 찾아 이어가는 범위가 넓어진 것처럼 보였습니다.

## 바뀌지 않은 기본

성능과 작업 방식은 많이 달라졌지만 기본적인 큰 맥락은 변하지 않은 것 같습니다. 원하는 결과를 얻으려면 프롬프트를 확실하게 주고, 프로젝트 구조를 정리하며, 파일과 기능을 계층적으로 관리해야 합니다. 이번 첫 프롬프트에도 만들 대상, 입력 방식, 실행 요구, 코드와 주석의 언어를 분명히 적었습니다.

AI가 알아서 잘 마무리해준다고 해도 결과를 직접 실행하고 확인하는 과정은 여전히 필요합니다. 이번에는 숫자 5를 직접 그려 인식 결과와 98.8% 신뢰도, 모델 입력 이미지를 확인했습니다. Auto mode가 작업 중간을 편하게 만들어줘도 최종 결과가 목적에 맞는지 판단하는 일까지 대신해주는 것은 아닙니다.

이번 3주차 미션을 통해 AI의 발전을 확실히 느꼈지만, 좋은 프롬프트와 구조적인 프로젝트 관리, 마지막 검증이 중요하다는 점도 다시 확인했습니다.
